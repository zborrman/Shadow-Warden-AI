...
| **Deploy & Infrastructure Monitoring** | `GET /deploy/status` reports on 10 of the stack's 24 services (warden, redis, postgres, minio, prometheus, grafana, analytics, arq, proxy, dashboard) — `proxy` and `dashboard` are assumed rather than probed. It used to probe an `app` service that `docker-compose.yml` does not define, so that entry could never come back healthy. SOC Dashboard `/platform/status` page auto-refreshes every 30s. Portal `/deployment/` infrastructure view with quick-links to Grafana, Jaeger, MinIO, and Prometheus. |
