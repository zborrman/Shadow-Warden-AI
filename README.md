...
| **Deploy & Infrastructure Monitoring** | `GET /deploy/status` reports on 11 of the stack's 24 services (warden, redis, postgres, minio, prometheus, grafana, app, analytics, arq, proxy, dashboard) — `proxy` and `dashboard` are assumed rather than probed, and `app` is a service `docker-compose.yml` does not define, so that entry cannot come back healthy. SOC Dashboard `/platform/status` page auto-refreshes every 30s. Portal `/deployment/` infrastructure view with quick-links to Grafana, Jaeger, MinIO, and Prometheus. |
...
| **Roadmap — Cryptography** | CR-13: ML-KEM-1024 upgrade path (FIPS 203 Level 5) · CR-14: HSM integration — PKCS#11 bridge for sovereign key material · CR-15: Certificate-pinned MASQUE tunnels — TOFU → CA-signed upgrade path. All Enterprise / planned. |
| **Roadmap — Compliance & Privacy** | CP-22: ISO 27001 Annex A control mapping · CP-23: HIPAA technical safeguards attestation · CP-24: NIS2 Directive compliance report · CP-25: Continuous compliance scoring dashboard (Pro+) · CP-26: Data retention policy enforcement — tenant-configurable per data_class (Community+). All planned. |
| **Roadmap — Integrations** | IN-14: VS Code inline risk annotation (Individual+) · IN-15: GitHub Actions pre-commit hook (Pro+) · IN-16: Jira auto-create security tickets (Pro+) · IN-17: Microsoft Teams /warden command (Pro+) · IN-18: Notion page scanner (Community+) · IN-19: STIX/TAXII feed consumer (Enterprise) · IN-21: OpenTelemetry WardenSpanProcessor (Pro+) · IN-22: MISP syslog bridge (Pro+). All planned. |
| **Roadmap — Dashboard/Billing/Infra** | FE-12: SOC Dashboard DNS live · FE-13–14: Analytics API + mobile-responsive SOC · FE-15: Onboarding wizard · FE-16: Dark/light theme toggle · FE-17: /pricing interactive page · OB-26–28: WebSocket stream, Grafana unified, React Native app · IF-19–20: Kubernetes HPA, multi-region active-active · BL-19: Request overage billing · BL-20: Obsidian Business Pack add-on. |
