...
| Feature | Description |
|---------|-------------|
| **Security Hardening — P0/P1 Fixes** | **Fail-closed auth (#11)**: startup raises `RuntimeError` if `WARDEN_API_KEY` and `WARDEN_API_KEYS_PATH` are both unset (prevents accidental open-auth production deploy; `ALLOW_UNAUTHENTICATED=true` required for dev). **VAULT_MASTER_KEY validation (#1)**: Fernet key validated at boot — malformed key halts startup with a clear recovery message. **Shadow ban randomness (#3)**: `_GASLIGHT_POOL` expanded 6→30 entries; `secrets.choice()` replaces deterministic hash-mod selection, preventing response fingerprinting. **CPT drift gate (#6)**: `calibrate_from_logs()` rejects any parameter update shifting >25% from prior — blocks slow-burn poisoning via coordinated borderline attacks. **Evolution regex ReDoS gate (#2)**: AI-generated `regex_pattern` rules validated before persistence — compile check + 0.3s degenerate-string timeout + nested-quantifier heuristic. |
...
| **Q1.3 Secrets Rotation Alerts** | `warden/api/rotation.py` — tracks 8 API secrets by SHA-256 digest prefix in Redis (`warden:key_age:{digest}`). `GET /admin/rotation/status` returns age/status per key and fires Slack alert on WARNING (≥75 days) or EXPIRED (≥90 days). `POST /admin/rotation/record` resets age clock. `POST /admin/rotation/rotate-alert` manual Slack trigger. Admin key required (`X-Admin-Key` header). |
| **Q2.4 Browser Extension Risk Scanner** | `warden/api/extension_risk.py` — `POST /scan/extensions` accepts Chrome extension metadata, scores against known-risk DB (7 extensions: malicious data harvesters, MITM proxies) plus permission heuristics (`webRequestBlocking`, `nativeMessaging`, `debugger`, `proxy`). `browser-extension/background.js` scans `chrome.management.getAll()` on install + every 6h. Notifications on HIGH/CRITICAL findings. |
| **Q2.6 Clipboard Monitor** | `browser-extension/content.js` — paste event listener (capture phase, `passive:true`). Payloads ≥500 chars forwarded to `POST /filter` via background SW. Advisory toast on BLOCK or MEDIUM verdict. `manifest.json` gains no new permissions (uses existing `POST /filter` flow). |
| **Q3.7 SMB Compliance Report** | `warden/api/compliance_report.py` — `GET /compliance/smb-report` (JSON), `/html` (print-ready), `/pdf` (reportlab optional, HTML fallback with `X-Report-Format` header). Aggregates analytics logs for GDPR Art.5/30/35 checklist (8 items) + FZ-152 Art.18/19/21 (3 items). KPI grid: total/blocked/PII-hits/anonymisation-rate. Configurable via `ORG_NAME`, `TENANT_ID`, `DATA_RESIDENCY_JURISDICTION`, `RETENTION_DAYS`. |
| **Q4.10 Agent Action Whitelist** | `warden/agentic/action_whitelist.py` — `ActionWhitelist` class backed by SQLite (shares `AgentRegistry` DB). Per-agent rules: `http_method` glob, `endpoint_glob` (fnmatch), `max_rps`. `check_action(agent_id, method, endpoint)` returns `(allowed, reason)`. 1-second sliding window rate limiter in `agent_action_rate` table. Open policy logged as warning when no rules exist. |
| **Q4.12 Indirect Prompt Injection in Files** | `warden/api/file_scan.py` — three regex patterns detect injection hidden in HTML/binary: `_HTML_COMMENT_INJECTION_RE` (malicious `<!--` comments), `_HIDDEN_ELEMENT_RE` (CSS-hidden elements with `display:none`/`opacity:0`/`color:white`), `_META_INJECTION_RE` (poisoned `<meta>` tags). Wired for `.html`, `.htm`, `.svg`, `.xml` uploads. Sets `injection_found=True` → risk escalates to CRITICAL. |
| **C2 docx/xlsx Scanner** | `warden/api/file_scan.py` extended — `_extract_docx()` (python-docx: paragraphs + table cells) and `_extract_xlsx()` (openpyxl: all sheets, all cell values). Both fail-open to raw UTF-8 decode if library missing. `GET /filter/file/supported-types` now returns `"office": [".docx", ".xlsx", ".xls"]`. |
| **C3 Offline Mode** | `warden/offline.py` — `is_offline()` reads `OFFLINE_MODE` env var. When true: Evolution Engine disabled at startup (no LLM calls), `/health` response includes `"offline_mode": true`. Redis cache and S3 fail-open by design — no additional changes needed. |
| **C5 Email Guard** | `warden/api/email_guard.py` — `POST /scan/email` accepts subject/body/from_address/raw_headers. 5 social-engineering subject patterns, 6 injection body patterns, phishing link detection (defanged URL classifier). Calls `SecretRedactor` + `SemanticGuard`. Returns `safe`, `risk_level`, `findings`, `sanitized_body`. |
| **Dashboard SPA** | `landing/dashboard.html` — full Claude Design System SPA. Two screens: Dashboard (KPI cards, 4 Chart.js charts, live event feed, API key management modal) and API Docs. "Settings" tab is a nav link to `/enterprise-settings`. Accessible at `/dashboard` via Vercel rewrite. Added "Dashboard" button to main site nav. |
