...
**A second audience is not human.** The M2M Agentic Marketplace is consumed by autonomous agents: a foreign agent's first contact is `GET /.well-known/agent.json` or `GET /marketplace/protocol`, not a page. For that audience the "interface" is a JSON manifest, and every design rule below inverts — there is no hierarchy to scan, no affordance to discover, and a field that is merely *present* is read as a field that is *true*. The manifest once advertised a two-major-version-old identity and a disabled payment gate as `enabled: "false"` — a string, which every consumer language reads as true. Machine-readable surfaces are product surfaces and carry the same honesty obligation as the pages; they just fail more quietly. `[repo: warden/discovery/well_known.py, test_agent_card_truth.py]`
...
- **Machine-readable surfaces are surfaces:** `/.well-known/agent.json`, `/.well-known/mcp.json`, `GET /marketplace/protocol`, `site/public/openapi.json`, and one markdown body per route served by `Accept: text/markdown` negotiation on the page URL itself. Each is versioned, each is published on three surfaces that must agree (app, apex, www), and each has been wrong in production at least once. `[repo: edge/negotiate.mjs, test_site_agent_readiness.py]`
