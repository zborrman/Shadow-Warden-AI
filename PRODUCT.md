...
- **Machine-readable surfaces are surfaces:** `/.well-known/agent.json`, `/.well-known/mcp.json`, `GET /v1/marketplace/protocol`, `site/public/openapi.json`, and one markdown body per route served by `Accept: text/markdown` negotiation on the page URL itself. The API surfaces are versioned — `/v1/...` canonical, the unversioned spelling a deprecated alias — while `/.well-known/*` is deliberately **not**, because it is how a client discovers which versions exist. Each is published on three surfaces that must agree (app, apex, www), and each has been wrong in production at least once. `[repo: edge/negotiate.mjs, test_site_agent_readiness.py]`
