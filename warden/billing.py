...
warden/billing.py
──────────────────
Per-tenant cost aggregation and quota enforcement.

Closes the "billing dead-end" gap (v0.5 Gap 6): token_cost_usd has always
been written per-request to logs.json but was never aggregated or capped.
This module provides the aggregation engine and the quota enforcement gate.

How it works
────────────
1.  A 60-second background loop calls ``aggregate_from_logs()`` which reads
    new NDJSON lines from LOGS_PATH after the stored watermark timestamp,
    groups them by (date, tenant_id), and upserts rolling totals into the
    ``billing_daily`` SQLite table.

2.  ``is_quota_exceeded(tenant_id)`` checks the current calendar-month total
    against the tenant's ``tenant_quotas`` row.  Returns True if the cap is
    reached.

3.  ``_run_filter_pipeline()`` in main.py calls ``is_quota_exceeded()`` early
    in the pipeline (after IP block, before auth) and raises HTTP 402 when
    over quota.

Tables
──────
  billing_daily   Aggregated (date, tenant_id) rows — upserted by the loop.
  tenant_quotas   Monthly USD cap per tenant.
  agg_watermark   Single row recording the last processed log timestamp.
...
class BillingStore:
    ...
    def get_usage(
        ...
        tokens    = int(row[3])
        quota     = self.get_quota(tenant_id)
        quota_remaining = round(quota - cost_usd, 6) if quota is not None else None
