...
# ── Shadowed routes ──────────────────────────────────────────────────────────
#
# Two handlers registered on the same METHOD + path. FastAPI matches the first
# one registered, so the second is unreachable — and, being unreachable, it stops
# telling the truth about what that URL does. This has bitten twice already:
# /ws/events served an unauthenticated duplicate in production (#237), and six
# /communities routes in warden/api/communities_v2.py sat dead behind
# warden/communities/router.py with weaker auth than the versions that answer,
# so a change to mount order would have silently downgraded them.
#
# Detection reuses the same pollution-immune measurement as the inventory test:
# a route string appearing under two different modules is a shadow.

# Known and accepted, with the reason. Anything not listed here fails.
_ALLOWED_SHADOWS = {
    # `/.well-known/agent.json` is claimed by the A2A v1.0 spec (agent card) and
    # by the marketplace protocol manifest, whose alias is registered first and
    # therefore serves. Production returns the marketplace manifest, so the A2A
    # agent card is unreachable and A2A discovery is broken — but the site
    # advertises this URL via <link rel="agent-protocol"> and external agents
    # already consume the marketplace shape, so changing what it returns is a
    # protocol decision, not a cleanup. Tracked, deliberately not auto-fixed.
...
def test_no_new_shadowed_routes():
    ...
    for module, routes in groups.items():
        for route in routes:
            owners.setdefault(route, []).append(module)

    shadowed = {
        route: sorted(mods)
        for route, mods in owners.items()
        if len(mods) > 1 and route not in _ALLOWED_SHADOWS
    }

    detail = "\n".join(
        f"    {route}\n        " + "\n        ".join(mods)
        for route, mods in sorted(shadowed.items())
    )
    assert not shadowed, (
        "Route registered by more than one module — only the first-registered "
        "handler runs, the rest are unreachable:\n"
        f"{detail}\n"
        "Delete the duplicate, or add it to _ALLOWED_SHADOWS with the reason."
    )
