...
class TestNoPlaceholderIdentity:
    """A machine-readable identity document must not publish a value nobody filled in.

    `/.well-known/did.json` shipped for months with its only verification method
    set to the literal string `REPLACE_WITH_PLATFORM_ED25519_PUBKEY_BASE58` —
    served with HTTP 200 on www, resolved from the apex, and read by agents as
    the platform's signing key. No such key existed anywhere in the codebase.
    Existence checks elsewhere in this file passed the whole time, because the
    file *was* published; nothing read what it said.
    """

    _MARKERS = ("REPLACE_WITH", "YOUR_", "CHANGEME", "TODO_FILL", "<placeholder>")

    @pytest.mark.parametrize("base", [_PUBLIC, _LANDING], ids=["site/public", "landing"])
    def test_well_known_documents_carry_no_placeholder(self, base):
        well_known = base / ".well-known"
        if not well_known.is_dir():
            pytest.skip(f"{well_known} not present in this checkout")
        offenders = []
        for path in sorted(well_known.rglob("*")):
            if not path.is_file():
                continue
            text = path.read_text(encoding="utf-8", errors="ignore")
            hits = [m for m in self._MARKERS if m in text]
            if hits:
                offenders.append(f"{path.relative_to(base)}: {hits}")
        assert not offenders, "placeholder values published as identity data: " + "; ".join(offenders)

    def test_did_document_asserts_no_key_it_cannot_back(self):
        """A verification method is optional (W3C DID Core §5.1); a fake one is not."""
        import json

        doc = json.loads((_PUBLIC / ".well-known" / "did.json").read_text(encoding="utf-8"))
        for vm in doc.get("verificationMethod", []):
            key = vm.get("publicKeyMultibase") or vm.get("publicKeyBase58") or ""
            assert key and not any(m in key for m in self._MARKERS), vm
