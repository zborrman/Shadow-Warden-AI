...
def _append_stix(ueciid: str, tenant_id: str, community_id: str) -> str:
    ...
    from warden.db.connect import open_db  # noqa: PLC0415
    try:
        with open_db(
            "sep", _SEP_DB_PATH, turso_name="sep", module_default_path=_SEP_DB_PATH
        ) as con:
            # Real schema (registered by warden.communities.stix_audit under the
            # shared "sep" db_key): chain_id/transfer_id/bundle_hash, not this
            # module's own entry_id/no-hash-column shape it used to maintain
            # independently -- that was a second, incompatible CREATE TABLE for
            # the same table name (order-dependent schema race).
            row = con.execute(
            ...
            con.execute(
                ...
                "VALUES (?,?,?,?,?,?,?,?)",
                (entry_id, community_id, entry_id, bundle, bundle_hash, prev_hash, seq, now),
            )
        return entry_id
