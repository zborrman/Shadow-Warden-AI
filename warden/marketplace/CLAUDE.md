...
## M2M Base Endpoints (all three required)

```
POST /marketplace/register    ← first-contact DID registration (api.py wrapper → api_agents.py)
GET  /marketplace/protocol    ← capability manifest discovery
POST /marketplace/action      ← unified action dispatcher
```
...
| `buyer_agent.py` | `BuyerAgent`: `search_and_buy()` (fairness guard), `auto_buy()`, `search_assets()` |
...
| `api.py` | `POST /register`, `GET /protocol`, `POST /action`, `POST /analytics/query` |
...
3. **`POST /analytics/query` is SELECT-only.** Any non-SELECT statement returns `{"error": "..."}` immediately. The `caller_agent_id` field scopes results — a query referencing another agent's DID is rejected.
...
Test file: `warden/tests/test_marketplace_m2m.py` (18 tests, all must pass)
...
Coverage gate: `--cov-fail-under=79` (run without `--no-cov` for CI).
...
| `AGENTS_MD_PATH` | `data/AGENTS.md` | Loop state file path |
