...
26. **An agent gets its autonomy policy from KYA screening — nothing else grants one.** `check_action()` treats "no policy" as L1 REQUIRE_APPROVAL, so for a long time *every* agent was permanently L1 and flipping `AUTHORIZE_PAYMENT_ENFORCED=true` would have returned HTTP 400 on 100% of purchases and clearings — a kill switch, not a tightening. `kya.py::screen_agent()` now calls `autonomy.ensure_default_policy()` when an agent reaches **VERIFIED**, and `revoke_agent()` calls `delete_policy()` (a revoked agent must not keep spend rights — `check_action()` reads the policy, not the KYA status). The grant is L2 with an approval threshold, never L3, never unlimited; it **never overwrites** an existing policy, so an operator's deliberate L1 lock-down wins. Tune with `KYA_DEFAULT_AUTONOMY_LEVEL` (`0` disables the grant entirely), `KYA_DEFAULT_APPROVAL_ABOVE_USD`, `KYA_DEFAULT_DAILY_USD`, `KYA_DEFAULT_MAX_SPEND_USD`. Do not "fix" a blocked agent by making no-policy resolve to ALLOW — that deletes the safe default the whole autonomy model rests on.
27. **Never do schema work on the per-connection path.** `_conn()` in `listing.py`
   and `agent.py` used to run six `_migrate_*` helpers and `_ensure_columns` on
   *every* connection — nine of those statements are `ALTER TABLE ... ADD COLUMN`
   calls that are expected to fail because the column already exists. On local
   SQLite that is microseconds and invisible. **This database is on Turso in
   production** (`TURSO_URL_MARKETPLACE`), where every statement is a separate
   HTTPS request, so it cost ~16 round trips: `GET /marketplace/listings`
   answered an empty list in **9.5 s** against a `/health` of 10 ms, and
   `/agents` in 2.8 s. Schema work belongs to the database, not the connection:
   run it once per process per db path (`_run_migrations_once`,
   `reset_column_memo`), the same shape `ddl_registry.ensure_schema` already
   uses. Guarded by `warden/tests/test_marketplace_migration_memo.py`, which
...
| `KYA_DEFAULT_AUTONOMY_LEVEL` | `2` | Autonomy level granted when an agent reaches KYA VERIFIED. `0` disables the grant (agents stay L1 = can do nothing once enforcement is on) |
| `KYA_DEFAULT_APPROVAL_ABOVE_USD` | `10.0` | L2 threshold on the granted policy — spend at or above this routes to a human |
| `KYA_DEFAULT_DAILY_USD` | `100.0` | 24h rolling limit on the granted policy |
| `KYA_DEFAULT_MAX_SPEND_USD` | `10.0` | Per-session ceiling on the granted policy |
