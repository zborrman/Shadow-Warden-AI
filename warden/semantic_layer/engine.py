...
for _raw in [
    ...
    # -- M2M Marketplace -- SEM-02, re-pointed by MP-5 (2026-08-01) ------------
    # These ten models were defined against mp_* tables filled by a projector in
    # m2m_store/analytics.py whose record_* functions had NO production caller --
    # only a test. Every model therefore returned an empty result set in
    # production, silently, as "no data" rather than "not wired".
    #
    # Owner decision D-2: re-point at the real marketplace_* tables and delete
    # the projector, rather than wiring a fifth order-shaped writer while FT-6
    # Phase C is mid-cutover.
    #
    # Two models are GONE rather than re-pointed, because no table backs them:
    # `marketplace_reputation` (TrustRank is computed on demand by
    # ReputationEngine and never persisted) and `marketplace_cross_chain` (no
    # such table exists anywhere in the codebase). Serving them would have meant
    # inventing a source. Dimensions and metrics with no real column are dropped
    # for the same reason -- a missing column is a broken query, not a null.
    {
        "id": "marketplace_listings",
        "name": "Marketplace Listings",
        "source_table": "marketplace_listings",
        "description": "M2M Marketplace product listings - one row per listing.",
        "metrics": [
            ...
            {"name": "avg_price",       "expression": "AVG(price_usd)",                                      "description": "Average listing price", "format": "currency"},
            {"name": "total_value",     "expression": "SUM(price_usd)",                                      "description": "Total value of all listings", "format": "currency"},
        ],
        "dimensions": [
            {"name": "listing_id",      "column": "listing_id",       "description": "Listing ID"},
            {"name": "seller_agent_id", "column": "seller_agent",     "description": "Seller agent"},
            {"name": "community_id",    "column": "community_id",     "description": "Community"},
            {"name": "tenant_id",       "column": "tenant_id",        "description": "Tenant"},
            {"name": "asset_type",      "column": "asset_type",       "description": "Asset type"},
            {"name": "date",            "column": "DATE(listed_at)",  "description": "Listing date"},
    ...
    },
    {
        ...
        "source_table": "marketplace_purchases",
        "description": "Completed M2M trades - one row per purchase.",
        "metrics": [
            {"name": "total_trades",     "expression": "COUNT(*)",                    "description": "Total completed trades"},
            {"name": "trade_volume_usd", "expression": "SUM(price_paid)",             "description": "Total trade volume", "format": "currency"},
            {"name": "avg_trade_value",  "expression": "AVG(price_paid)",             "description": "Average trade value", "format": "currency"},
            {"name": "unique_buyers",    "expression": "COUNT(DISTINCT buyer_agent)", "description": "Unique buyers"},
        ],
        "dimensions": [
            {"name": "trade_id",        "column": "purchase_id",        "description": "Purchase ID"},
            {"name": "buyer_agent_id",  "column": "buyer_agent",        "description": "Buyer agent"},
            {"name": "seller_agent_id", "column": "seller_agent",       "description": "Seller agent"},
            {"name": "listing_id",      "column": "listing_id",         "description": "Source listing"},
            {"name": "status",          "column": "status",             "description": "Purchase status"},
    ...
    },
    {
        ...
        "source_table": "marketplace_escrow",
        "description": "M2M escrow contracts - one row per escrow.",
        "metrics": [
            ...
            {"name": "escrow_value_usd", "expression": "SUM(amount_usd)",                                                   "description": "Total value held in escrow", "format": "currency"},
        ],
        "dimensions": [
            {"name": "escrow_id",       "column": "escrow_id",        "description": "Escrow ID"},
            {"name": "buyer_agent_id",  "column": "buyer_agent",      "description": "Buyer agent"},
            {"name": "seller_agent_id", "column": "seller_agent",     "description": "Seller agent"},
            {"name": "listing_id",      "column": "listing_id",       "description": "Source listing"},
    ...
    },
    {
        ...
        "source_table": "marketplace_negotiations",
        "description": "Price negotiation sessions between M2M agents.",
        "metrics": [
            {"name": "total_negotiations", "expression": "COUNT(*)",           "description": "Total negotiation sessions"},
            {"name": "avg_rounds",         "expression": "AVG(round_count)",   "description": "Average negotiation rounds"},
            {"name": "avg_price",          "expression": "AVG(current_price)", "description": "Average agreed price", "format": "currency"},
        ...
        "dimensions": [
            {"name": "negotiation_id",  "column": "negotiation_id",   "description": "Negotiation ID"},
            {"name": "buyer_agent_id",  "column": "buyer_agent",      "description": "Buyer agent"},
            {"name": "seller_agent_id", "column": "seller_agent",     "description": "Seller agent"},
            {"name": "listing_id",      "column": "listing_id",       "description": "Listing under negotiation"},
            {"name": "date",            "column": "DATE(created_at)", "description": "Negotiation date"},
            {"name": "status",          "column": "status",           "description": "Outcome (open/accepted/rejected/expired)"},
        ],
    },
    {
        "id": "marketplace_governance",
        "name": "Marketplace Order Proposals",
        "source_table": "marketplace_proposals",
        "description": "Structured M2M order proposals (quantity + SLA + max price).",
        "metrics": [
            {"name": "total_proposals",     "expression": "COUNT(*)",                                              "description": "Total proposals"},
            {"name": "accepted_proposals",  "expression": "SUM(CASE WHEN status = 'accepted' THEN 1 ELSE 0 END)",  "description": "Accepted proposals"},
            {"name": "avg_quantity",        "expression": "AVG(quantity)",                                         "description": "Average quantity requested"},
            {"name": "avg_max_price",       "expression": "AVG(max_price_per_unit)",                               "description": "Average max unit price", "format": "currency"},
        ],
        "dimensions": [
            {"name": "proposal_id",     "column": "proposal_id",      "description": "Proposal ID"},
            ...
            {"name": "listing_id",      "column": "listing_id",       "description": "Listing"},
    ...
    },
    {
        ...
        "source_table": "marketplace_agents",
        "description": "M2M Marketplace registered AI agents registry.",
        "metrics": [
        ...
        "dimensions": [
            {"name": "agent_id",      "column": "agent_id",         "description": "Agent DID"},
            {"name": "community_id",  "column": "community_id",      "description": "Community"},
            {"name": "tenant_id",     "column": "tenant_id",        "description": "Owning tenant"},
            {"name": "status",        "column": "status",           "description": "Agent status"},
            {"name": "date",          "column": "DATE(created_at)", "description": "Registration date"},
    ...
    },
    {
        ...
        "source_table": "marketplace_assets",
        "description": "Tokenized assets traded in the M2M Marketplace.",
        "metrics": [
            {"name": "total_assets",   "expression": "COUNT(*)",                         "description": "Total tokenized assets"},
            {"name": "unique_sellers", "expression": "COUNT(DISTINCT seller_agent_id)",  "description": "Distinct asset owners"},
        ],
        "dimensions": [
            {"name": "asset_id",        "column": "asset_id",         "description": "Asset UECIID"},
            {"name": "seller_agent_id", "column": "seller_agent_id",  "description": "Owning agent"},
            {"name": "community_id",    "column": "community_id",     "description": "Community"},
            {"name": "tenant_id",       "column": "tenant_id",        "description": "Tenant"},
    ...
    },
    {
        ...
        "source_table": "maestro_flags",
        "description": "MAESTRO security flags raised against M2M Marketplace agents.",
        "metrics": [
            {"name": "total_flags",    "expression": "COUNT(*)",                                     "description": "Total MAESTRO flags"},
            {"name": "avg_score",      "expression": "AVG(score)",                                   "description": "Average threat score"},
            {"name": "high_threats",   "expression": "SUM(CASE WHEN score >= 0.7 THEN 1 ELSE 0 END)","description": "High-scoring threats (>= 0.7)"},
            {"name": "flagged_agents", "expression": "COUNT(DISTINCT agent_id)",                     "description": "Distinct flagged agents"},
        ],
        "dimensions": [
            {"name": "flag_id",   "column": "flag_id",          "description": "Flag ID"},
