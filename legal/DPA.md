...
**Between:** Shadow Warden AI ("**Data Processor**", "**we**", "**us**")

> **This is a template, and it cannot be executed yet.** No legal entity is
> registered, so there is no party able to sign as Processor. Incorporation is
> planned once the service is fully live; the entity name and registered address
> belong in the line above at that point. Until then this document states the
> terms intended, not an agreement in force.
...
| _(none engaged)_ | — | Billing — **no payment provider is currently connected**; neither Stripe nor Lemon Squeezy credentials exist in production, so no billing data reaches any third party. Add the provider, its country and its transfer basis here when one is connected. | — |
