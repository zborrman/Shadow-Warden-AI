...
Measured on production 2026-09-05, `grafana.db` byte 18 is 1 — a rollback
journal — and no `-wal` or `-shm` sidecar exists, so WAL has never been engaged
even transiently. `SQLITE_BUSY` "database is locked" errors were still being
logged on 2026-09-04, which is the symptom the setting was meant to remove.
...
#: 1 = rollback journal, 2 = WAL. https://sqlite.org/fileformat.html#the_database_header
...
def journal_mode(path: Path) -> str:
    ...
    if not header.startswith(_HEADER_MAGIC):
        return "not-sqlite"
    return MODES.get(header[_WRITE_VERSION_OFFSET], f"unknown({header[_WRITE_VERSION_OFFSET]})")
...
def _targets(args: list[str]) -> list[Path]:
    ...
    for arg in args:
        p = Path(arg)
        if p.is_dir():
            out.extend(sorted(q for q in p.rglob("*.db") if q.is_file()))
        elif p.exists():
            out.append(p)
        else:
...
def main(argv: list[str]) -> int:
    ...
    paths = _targets([a for a in argv if not a.startswith("--")])
    if not paths:
    ...
    if expect_wal and bad:
        print(
            f"\n{bad} file(s) are not in WAL. A setting that enables WAL is not "
