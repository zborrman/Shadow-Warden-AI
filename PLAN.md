...
| 1b | Give CI the credential to ship the Worker | `cf-marketplace-deploy` already knows when the edge is behind — it compares the `VERSION` constant with what `/health` answers — and cannot act, because `CLOUDFLARE_API_TOKEN` is not a repository secret. It therefore fails on every `main` push where source and edge differ, which is correct and useless. Add the token (Workers Scripts: Edit, Workers Routes: Edit on the zone) or accept that shipping this Worker is a human step and say so in one place |
