...
| 1b | ~~Give CI the credential to ship the Worker~~ | **Done 2026-09-20.** `cf-marketplace-deploy` (#512) ships the Worker on a `main` push and proves the edge moved; `CLOUDFLARE_API_TOKEN` is set, scoped to Workers Scripts: Edit, Account Settings: Read and Workers Routes: Edit on one zone. #514 gated the *preflight* Worker behind `vars.CF_PREFLIGHT_DEPLOY` first, so adding the credential could not put an unexercised Worker in front of the production API |
