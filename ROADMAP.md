...
| FE-52 | Shadow Agentic Container (SAC) — Runtime Security Core: Inner Warden execution guard (`warden/sac/guard.py`) screens every agent tool call (SSRF/exfil URL block fail-CLOSED via `net_guard`, secret-path denylist), first GSAM producer emitting metadata-only observations (fail-OPEN); wired into SOVA `traced_dispatch` + `staff_dispatch` (closes agent-supplied-URL SSRF gap at `BrowserSandbox`); Hermes JIT credential lease (`warden/gsam/jit_lease.py`) — single-use HMAC leases on `gsam_leases`, fail-CLOSED (503 without key), secret never in response, `/gsam/lease*` API (4 endpoints); 18 tests | v7.8 | All | ✅ |
