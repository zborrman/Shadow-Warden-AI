...
| H-9 | **No test asserts a marketplace write route actually rejects a caller.** `test_marketplace_route_auth.py` inspects the dependency callables attached to each route — it proves `require_api_key` is *wired*, never that it *denies*. And the documented way to run the suite (§5, and `warden/marketplace/CLAUDE.md`) sets `WARDEN_API_KEY=""`, which puts `require_api_key` in dev mode where a missing header is accepted. So the whole marketplace suite runs with the guard it depends on switched off. | Wiring and enforcement are different claims. A dependency can be present and still permit everyone — which is how three of the four anti-patterns in `Rule.md` §29.1 work. | One selected test with a non-empty `WARDEN_API_KEY` asserting a protected write route rejects both a missing and a wrong `X-API-Key`. It does not need to cover every route; it needs to prove the mechanism denies at all. |
| H-8 | **The marketplace Worker is guarded only at registration.** `workers/shadow-warden-marketplace/` is deployed at `marketplace.shadow-warden-ai.com` and re-implements register / listings / negotiate / clear over KV, in TypeScript. #506 closed the identity defects there (takeover, free-form DID, empty-secret admin gate) and added the first ratchet over it — `test_marketplace_worker_identity.py`, which runs `did.ts` under node against `pubkey_to_agent_id()` for cross-implementation equivalence, plus source ratchets for what only workerd can observe. | Everything else on that surface is still unguarded: `sendOffer`, `acceptOffer`, `rejectOffer` and `POST /clear` act on a body-supplied DID with no signature, and every ratchet in §3 enumerates **FastAPI** routes, so none of them sees the Worker. **A merged fix is also not a deployed one** — no workflow runs `wrangler deploy` for it. | Port `_assert_actor()`'s actor proof to the Worker, or retire the second implementation. See `Rule.md` §29.2 and the first item on the path to P1a exit in `PLAN.md`. |
