...
| ~~H-9~~ | **Closed.** `test_marketplace_auth_actually_denies.py` configures a real `WARDEN_API_KEY`, shuts `ALLOW_UNAUTHENTICATED`, and sends three requests to a protected write route: missing key **401**, wrong key **401**, configured key **422** (auth ran, passed, validation rejected the deliberately invalid body). The fourth assertion matters as much as the first two — a gate that refuses its own credential passes "denies anonymous" and "denies wrong" while protecting nothing usable. Runs in a subprocess because `_VALID_KEY` binds at import, and fails loudly rather than silently when the key does not reach the module. Verified non-vacuous by running it under the suite's own env, where it reports exactly that. |
