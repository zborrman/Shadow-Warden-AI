...
|---|---|---|---|
| ~~H-1~~ | **This row was wrong: the guard existed.** `test_posture_flags_reach_the_container.py` (PR #320, 2026-08-11) already pinned eight posture flags when this entry was written as "not built" — the absence was asserted from a search that missed it, the exact failure this file warns about. | The real gap was narrower: `ESCROW_SETTLE_CHAINS`, the on-chain settlement gate, was passed through but never pinned, so a compose edit could drop it and turn the first mainnet trade into a no-op that looked configured. | **Closed.** Added to the guard, with the defaults test taught that a list flag's "off" is empty, never a boolean (rule 31). Also records the opposite case: `X402_REQUIRE_SIGNED_PAYMENT` defaults to `true` in code and is deliberately **not** passed through — adding it would make the payer-signature check switchable from `.env` — and a test fails if it gains a passthrough or loses its secure default. Both verified by mutation. |
