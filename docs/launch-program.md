...
| Escrow contract | Drafted 2026-08-24: `contracts/Escrow.sol` — singleton per chain, ERC-20 (USDC), with the six functions `warden/marketplace/escrow.py` already calls, taken from the caller rather than invented. `contracts/escrow.abi.json` is hand-written and checked against both the source and the call sites by `test_escrow_abi_matches_callers`; `scripts/build_escrow.sh` compiles it with a pinned solc in Docker so the deployed bytecode is reproducible. **Not compiled, not audited, not deployed** — the arbiter is a single key and should be a multisig before this holds meaningful value. |
