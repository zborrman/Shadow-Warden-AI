...
| FT-5 | Compliance: KYB behind KYA, sanctions at settlement, AML on journal stream, licensing-posture doc | 🟡 1/4 — `docs/licensing-posture.md` done: full money-flow inventory (subscription billing via Lemon Squeezy MoR, internal service credits, M2M marketplace escrow/clearing, x402 metering) traced to code, concluding the system's money handling is a closed-loop non-redeemable service-credit model (no MSB/money-transmitter registration warranted *as implemented*), conditional on 4 invariants (no redemption path, no third-party payout, fiat-in stays behind the MoR, escrow settles to internal accounts only) — explicitly flagged as a technical review for counsel, not legal advice. Highest-attention item for follow-up counsel review: §2.3 marketplace escrow (tenant-to-tenant, still closed-loop credits not fiat). KYB/sanctions/AML remain | — |
