...
| FT-4 | Settlement worker + nightly reconciliation + transactional outbox (absorbs NF-3) | ✅ done — slice 1 (`warden/workers/x402_settlement.py::settle_pending_deductions()`, x402 pending-deduction settlement), slice 2 (`warden/workers/ledger_recon_job.py::run_ledger_reconciliation()`, credit-drift gauge + alert), slice 3 (`warden/marketplace/clearing.py`, transactional outbox for the Postgres relay) + outbox retention/cleanup (`purge_relayed_outbox()`, weekly cron) all shipped and merged (details in prior status, superseded by this row). ✅ holds reconciliation done — the previously-deferred semantics gap (a naive drift check would flag every pre-dual-write hold as "missing from ledger," which is expected history, not a bug) resolved via **cutoff-timestamp** scoping (user's explicit choice over "flag as legacy" or "backfill then compare"): `warden/finops/ledger_recon.py::holds_drift()` only compares `sac_holds` rows created at/after `settings.ledger_holds_recon_cutoff_ts` (new config field, env `LEDGER_HOLDS_RECON_CUTOFF_TS`, default unset — the function is a no-op until an operator records the dual-write-enable time) against their `warden/ledger/holds.py` mirror, flagging `missing_in_ledger` or `status_mismatch` (live status maps HELD→HELD, COMMITTED→CAPTURED, RELEASED→VOIDED). New read accessor `sac/preflight.py::list_holds_since()`. Wired as `run_holds_reconciliation()` → `warden_ledger_holds_recon_mismatches` gauge + Slack alert, kept as a **separate** function/cron (`nightly_holds_recon`, daily 04:15 UTC) rather than folded into the existing credit-drift job, to avoid breaking `run_ledger_reconciliation()`'s already-tested flat return shape. Fail-soft throughout (never raises, ok-by-vacuity on any read error), matching `credit_drift()`'s posture. 22 new tests (holds_drift pure function, list_holds_since accessor, ARQ wrapper); 254 ledger/sac/preflight/config tests green; ruff+mypy clean; 4 ratchets green. This closes all previously-tracked FT-4 scope. | — |
