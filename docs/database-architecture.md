...
| **D-1b** | **Fold `data/init.sql` into Alembic.** Six tables exist only there; two are live code paths (`waitlist` ×1, `cert_applications` ×2 references) and four are unreferenced (`api_keys`, `filter_audit`, `dynamic_rules`, `hourly_stats`). init.sql runs *only* on first container init, so any later edit to it never reaches an existing volume — the same class of gap D-1 just closed. **Verify on the VPS first:** `psql -c "\dt warden_core.*"` — if `waitlist`/`cert_applications` are absent, those features are broken today too. | 1 d | Low | F1 (remainder) |
| **D-2** | **Cap Postgres memory** (`limits.memory`, `shared_buffers`) — the only unbounded service in compose. | 1 h | Low | F6 |
| **D-3** | **`filter_events` hypertable.** Timescale table + continuous aggregates; move the 25 `logs.json` readers behind one read helper. NDJSON stays the write-side journal. | 3 d | Low | F5, F8 |
| **D-4** | **Unify accounts.** One `users` + `api_keys` table in Postgres; `auth/router.py` and `portal_router.py` both read it. Migrate `warden_auth.db` rows. | 3 d | Med | F2, GDPR erasure correctness |
...
| **D-6** | **Class B → Postgres** (SEP + communities), resolving the duplicate `communities` schema. | 4 d | Med | F3 |
