...
| False-positive rate on benign traffic | `LIVE` | **0 of 35** benign prompts flagged, same run. This claim is real and currently under-sold. | Publish this number |
...
| Escrow with 48h delivery timeout | `SIMULATED` | Same endpoint reports `"settlement_mode": "simulated"`. The state machine runs; no value moves. | Label every escrow claim "simulated" until P1 |
| "Base Sepolia testnet + mainnet EVM" (`agentic.astro`) | `TESTNET` | `warden/web3/chains.py` defines Sepolia, Polygon Amoy and Arbitrum Sepolia. There is no mainnet chain. | Remove "+ mainnet EVM" |
...
- The falsifiable numbers are regenerated by `scripts/capability_probe.py`. If a
  number here disagrees with the probe, the probe is right.
