...
| FT-6 Phase C — reads cut over to the fielded order mirror | `BUILT` | Blocked on order volume, not on code. `phase_c_ready()` on production: `{'ready': False, 'reason': 'no orders were compared (orders_checked=0)'}`. The gate correctly refuses to read `ok: True` over zero rows as agreement — the FT-2 lesson, applied. | Unblocks in P3, when real orders exist to reconcile |
