...
| Jailbreak detection quality | `OVERSTATED` | **Public figure stays 36.2%** until plan-85 D-2's ≥150-prompt held-out eval exists. Internal measurement 2026-09-21, after the injection-family rules: **53 of 58** baseline prompts reach HIGH/BLOCK through `/filter` (was 21), 0 of 35 benign flagged — but that is in-sample, the rules were written with those misses visible. On a 28-prompt held-out set written first by the same author: 28 of 28 caught, 0 of 24 benign flagged. n=28 is too small to publish and not independent. Baseline in `warden/tests/adversarial/baseline.json` (missed 37 → 5), enforced by `test_adversarial_ratchet.py` and `test_injection_families_heldout.py`. Still missed: harmful requests wrapped in fiction, thought experiments or "educational" framing. | See §1.1 |
...
| Per-stage latency (`<2ms` topology, `<8ms` brain, …) — **142 occurrences** across `site/`, `landing/`, `portal/`, `dashboard/` and `docs/` | `UNMEASURED` → `BUILT` | The pipeline always measured this and never aggregated it: each stage writes into the `timings` dict returned as `processing_ms`, which lives for one response and is dropped. `warden_filter_stage_duration_seconds{stage}` now observes it at every exit of `/filter` and `ws_filter_stream` (`main.py::_observe_stage_timings`, buckets from 0.5 ms so a `<2ms` claim is resolvable). Not yet deployed — reads `BUILT` until it is scraped in production, then `LIVE` with the measured figures. The count read 19 because the ban listed only the spelled form (`sub-2ms`) while the site writes `<2ms`; `warden/tests/test_no_new_latency_claim.py` now matches the pattern in every notation and ratchets the total down. | **Measured 2026-08-31**, 201 production requests, every stage running on every request except `causal` (gray-zone, n=4). p95 in ms: `ml` **100**, `poison` **250**, `threat_vault` 25, `rules` 8, `obfuscation` 3, `redaction` 3, `phishguard` 3, `causal` 2, `topology` 1, `cache_check` 0.5. Against the published figures: the brain was sold at `<8ms` and measures a p95 of 100 ms with a p99 past the 1 s top bucket; rules `<3ms` measures 8; obfuscation and redaction `<1ms` measure 3; phishguard `<2ms` measures 3. Only `topology` (`<2ms` → 1 ms) held. `poison` is the second most expensive stage and appears on no published diagram at all. **The p99 tail is the pipeline, not middleware** — the `ml` stage alone exceeds a second, which answers the open question in the row above. The unmeasured badges are removed from `Architecture.astro`; 201 requests is enough to refute a claim, not yet enough to publish a replacement. |
| "P99 < 50ms" and end-to-end `/filter` latency (`docs/sla.md`, `trust.astro`) | `OVERSTATED` | The previous evidence in this row — "no `/filter` latency histogram exists in production" — **was wrong**. `http_request_duration_seconds{handler="/filter"}` has been scraped all along. Read from production on 2026-08-29, 60 observations: **mean 84.7 ms, p50 ≤ 25 ms, p90 ≤ 250 ms, p99 ≤ 1000 ms**. The advertised per-stage figures sum to under 20 ms and the SLA promises a p99 of 50 ms; the measured p99 is twenty times that. Caveats, stated because the number is small: 60 samples, coarse buckets above 250 ms, and this histogram times the whole HTTP request — auth, quota middleware and the known inline Evolution/Slack work on blocked requests (~1.7 s + ~1.8 s) sit inside it, so the tail is plausibly middleware rather than pipeline. That is a reason to attribute it with the new per-stage metric, not a reason to keep publishing 50 ms. | `docs/sla.md` now marks all three latency rows "target, not yet verified" and cites `warden_filter_duration_seconds{source="filter"}` — a metric that, until 2026-08-29, the SLA had named as its evidence without anything ever registering it. Set the numbers from that histogram once it has production data. |
...
| Multi-Modal Guard — CLIP image-jailbreak + Whisper audio screening (`image_guard.py`, `audio_guard.py`) | `BUILT` | Code and tests exist. The CLIP (`clip-vit-b-32`, ~600 MB) and Whisper weights are **not** baked into the runtime image (only MiniLM is), so with the guard enabled `_load_model()` fails `local_files_only` and every image/audio request fail-opens — `warden_stage_failopen_total{stage="image_guard",reason="model_not_loaded"}` was non-zero in production for weeks with no reader, because it is not one of the 9 core stages `/health/pipeline` watches. `IMAGE_GUARD_ENABLED` / `AUDIO_GUARD_ENABLED` now default **false** (S-3); a deployment that ships the weights turns them on. | Do not describe as active. Re-run `scripts/capability_probe.py` if the weights are added to the image. |
...
| `settlement_mode` as an honest signal | `LIVE` (repaired 2026-08-21, again 2026-08-24) | First repair distinguished mainnet from testnet — and was immediately defeated by configuration: `BASE_RPC_URL` defaults to the public endpoint `https://mainnet.base.org`, so a mainnet chain looks configured on every deployment, and production advertised `onchain` again with no contract and $0 settled. The field now asks capability before configuration: `warden/web3/smart_contract.py` deploys nothing and signs nothing (`real_address = None`, ABI not wired), so the honest answer everywhere today is `simulated`. It cannot read `onchain` until that stub is replaced. | Re-run `scripts/capability_probe.py` after any settlement change |
...
| "P99 < 50ms" (`docs/sla.md`) | `OVERSTATED` | **See the end-to-end latency row in §1.** The evidence that stood here — "no `/filter` latency histogram exists in production" — was wrong: `http_request_duration_seconds{handler="/filter"}` had been scraped all along, and read a p99 of roughly one second over 60 samples. This row is kept as a pointer rather than a second copy, because two rows on one claim is how the first one went stale. | Tracked in §1 |
| SLA service credits | `SELF-ATTESTED` (was `FABRICATED`; qualified 2026-08-29) | `docs/sla.md` §8 published a percentage schedule that reads as an automatic entitlement. Uptime **is** measured (`sova_sla_report`, error-budget burn monitors), but nothing turns a measured breach into a credit — no calculation, no issuance path, and no invoice to apply one to, since paid plans are not purchasable. §8 now carries a status note saying issuance is manual, pinned by `test_public_claims_reconciled.py`. ⚠️ This row was missed by the first claims sweep because the guard scoped itself to `site/`, `landing/`, `portal/` and `dashboard/` — **`docs/` is in the MkDocs nav and is just as public**. The guard now covers it. | Remove the note when a breach produces a credit on a real invoice without a human (P4 exit) |
...
| Evidence bundles offsite | `BUILT` (2026-08-22) | Was **not replicated**: the offsite bucket held 445 objects, every one under `backups/` — the nightly encrypted SQLite snapshots — and no `bundles/` prefix, so evidence lived on the single VPS and the R6 restore drill never covered it. `ship_evidence_bundles()` now mirrors the vault to `evidence-bundles/` on the offsite target, Fernet-encrypted with the snapshot key, incrementally, from the same 03:30 UTC job. **Still `BUILT`, not `LIVE`:** verify after the first nightly run that the prefix exists offsite and that one object decrypts to a bundle whose `bundle_hash` still verifies. |
