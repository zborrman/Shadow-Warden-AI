...
| Stage latency, in every notation (`sub-2ms`, `<2ms`, `in under 2ms`, `P99 < 50ms`) — **142 occurrences** across `site/`, `landing/`, `portal/`, `dashboard/` and `docs/` | `UNMEASURED` | No `/filter` latency histogram exists in production. The only duration metric scraped is `warden_arq_job_duration_seconds`. There is no instrument behind the number. The count read 19 while the ban listed only the spelled form: the site writes `<2ms`, so the guard passed over three quarters of it. Counted by `warden/tests/test_no_new_latency_claim.py`, which ratchets the total down and fails on any new one. | Instrument first, then publish |
