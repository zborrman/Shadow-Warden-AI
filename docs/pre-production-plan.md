...
|---|---|---|---|
| **SR-7.2** | Coverage floor 75% → 85% | 83%, open | `--cov-fail-under=83` since 2026-08-01 (79% → 83%). The last 2 points are non-security surface — `main.py`, `marketplace/`, `communities/`, `syndicates/` routers — so this ranks below SR-7.3. Raise with real tests, not coverage-boost files |
| **SR-7.3** | Extend mutation testing beyond the 2 seeded modules | four modules remediated; **re-measured 2026-08-31** | Baseline sweep 2026-08-02: 1482 mutants, 786 survived — `secret_redactor.py` 33 → 19 (#277), `auth_guard.py` 89 → ~51 (#280), `marketplace/x402_gate.py` 259 → 193 (#284, plus identity/amount propagation pinned in #288), `causal_arbiter.py` 405. The `causal_arbiter` remediation then landed as #285 (MLE success path, Laplace smoothing, block-rate/entropy-centre proxies, 4-dp rounding), #286 (every `_drift_ok` argument in the final and-chain) and #321 (the CPT gate link by link — short-circuit evaluation meant no test had ever reached a state where a *later* parameter's drift is what rejects). **Re-swept on `d6c72653` (2026-08-31): 652 mutants, 452 killed, `causal_arbiter.py` at 200 survivors** — 405 → 200, and below the 223 #285's title projected. Survivors by function: `calibrate_from_logs` 96, `arbitrate` 64, `online_update` 21, `reliability_curve` 14, `_sigmoid` 5. The drift-gate finding that motivated #286/#321 is closed, and the residue is not equivalent mutants: classifying every survivor by whether its added line only touches a `log.*` call or a docstring puts **26 at log-string-only and 174 at behaviour-changing** (a heuristic, so read 26 as a floor on the equivalents, not a proof). What those 174 mostly are is unasserted *output shape* rather than unguarded policy — `round(p_tool_risk, 4)` mutating to `round(p_tool_risk)` and the fail-open result's `p_reputation=0.0` mutating to `None` both survive because no test reads those fields — plus `online_update`'s learning-rate arithmetic (`eta = 1.0 / (1.0 + n)` → `2.0 / (1.0 + n)` survives; the 25% drift clamp absorbs it on the paths tests take). Linux/CI only (mutmut unsupported on native Windows); never run against this repo's own built image (`PYTHONPATH=/` shadows the mutant sandbox). Re-measured in a stock `python:3.11-slim` container with `[tool.mutmut]` narrowed to `causal_arbiter.py` + its three test files. |
