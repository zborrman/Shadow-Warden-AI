...
| **SR-7.3** | Extend mutation testing beyond the 2 seeded modules | harness fixed, full sweep run once, remediation open | Config migrated 2.x→3.x + 2 sandbox bugs fixed (see CLAUDE.md) — first-ever real run: **1482 mutants, 696 killed / 786 survived**. Survivors are NOT evenly spread: `secret_redactor.py` 33/137 (24%, already had registry-invariant tests from a prior slice), `semantic_guard.py` 0/7 (0%), `auth_guard.py` 89/227 (39%) — but `causal_arbiter.py` 405/652 (62%) and `marketplace/x402_gate.py` 259/459 (56%) are the newly-extended modules and carry nearly all the volume. Concentrated, specific, high-value finding: in `causal_arbiter.calibrate_from_logs`, **every argument of every `_drift_ok(...)` call in the final `and`-chained gate (covering `obfusc_pos`/`obfusc_neg`/`ers_center`/`entropy_center`) can be replaced with `None`, have its label dropped, or be reordered without any test noticing** — meaning no test reaches the point where an earlier check in the chain passes and a later parameter's specific drift value is what's being verified. This is exactly the "slow-burn coordinated poisoning" scenario the gate's own docstring says it exists to stop (CLAUDE.md's CPT drift gate #6): an attacker nudging one CPT parameter past the 25% threshold while keeping the others borderline-safe would not be caught by the current suite. In `x402_gate.py`, survivors concentrate in `require_payment` (113) and `deduct_payment` (49) — the functions gating actual money movement. Remediation (writing the ~786 missing assertions, prioritizing the drift-gate chokepoint and the two payment functions) is a separate, substantial follow-up — not attempted in the harness-repair slice. Linux/CI only (mutmut unsupported on native Windows); never run against this repo's own built image (`PYTHONPATH=/` shadows the mutant sandbox). |
