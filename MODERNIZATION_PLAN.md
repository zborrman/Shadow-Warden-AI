...
- ⏭️ Осталось: Фаза 1 — **1.1 (auth-middleware — инвазивно, отдельным циклом на свежем контексте)**, 1.4 (IDOR). Фаза 2 — 2.3 (IP-пиннинг), 2.4 (CORS `/ext/*`). Фаза 3 — 3.5-остаток, 3.8-остаток. Фаза 4 — 4.2-velocity (доп.), 4.3 GDPR-логи (`query_preview`/OIDC-email), 4.4 слабые ключи, 4.5 plaintext-секреты. Фазы 5–8.
