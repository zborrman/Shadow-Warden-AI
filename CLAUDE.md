...
**Version:** 7.7 · **License:** Proprietary · **Language:** Python 3.11+ · **Updated:** 2026-07-09
...
| `warden/gsam/schema.py` | GSAM (v7.7) — `Observation` metadata-only model + ClickHouse `gsam_observations` MergeTree DDL; no content field (`payload_kind` label only) |
| `warden/gsam/collector.py` | GSAM collector — bounded `gsam_emit()` (`put_nowait`, drop counter), daemon batch → ClickHouse, fail-open NDJSON spool + replay; `register_sink()` for rollup |
| `warden/gsam/clickhouse.py` | GSAM ClickHouse wrapper — guarded `clickhouse-connect` import, lazy `CREATE DATABASE/TABLE`, all methods fail-open |
| `warden/gsam/ingest.py` | GSAM ingest taps — the ONLY producer→collector bridge; GDPR field ALLOWLIST (strips `detail`/`query_preview`); `tap_agent_span/token_cost/billing_event/marketplace_action/mcp_call` |
| `warden/gsam/math.py` | GSAM math — pure functions: `session_cost`, `roi` (zero-cost guard), `frequency_vector`, `weighted_cosine_distance`, `ewma_drift`, `anti_inflation_score` (strong w=3 / weak w=1, ≥2 strong → critical) |
| `warden/gsam/drift.py` | GSAM drift — per-agent EWMA behavioural baseline in SQLite `gsam` DB (in-mem fallback); `update_drift()` quarantines at `gsam_drift_quarantine_threshold` |
| `warden/gsam/rollup.py` | GSAM rollup — collector sink folding flushed batches into `gsam_agent_stats` hourly buckets; keeps read APIs working without ClickHouse; `install()` at router-mount |
| `warden/gsam/quarantine.py` | GSAM quarantine — Redis `gsam:quarantine:{agent_id}` SETEX (in-proc fallback) + SQLite journal; `is_quarantined()` fail-open; marketplace + staff dispatch gates |
| `warden/gsam/jit_lease.py` | GSAM JIT lease — Hermes-style HMAC-signed single-use credential lease; **fail-CLOSED** (503 without `gsam_lease_secret`); Slack approval token; secret never in a response |
| `warden/gsam/reports.py` | GSAM read models — `heatmap()`, `agent_stats()`, `compliance_score()`; all from the SQLite rollup (ClickHouse-optional), fail-open |
| `warden/gsam/api.py` | GSAM router `/gsam/*` — observations, health, heatmap, agents/{id}/stats, compliance/score, quarantine list/release, lease request/approve/status |
...
- **Site version: `v7.7` Latest (GSAM — Global Statistic Agentic Marketplace: observation stream + drift/quarantine + JIT credential lease + anti-inflation compliance) / `v7.6` (Marketplace API Restored / Community API Restored / Route-Inventory Guard Hardening / CI Import Audit / MCP Config Fix) / `v7.5` (Layered Architecture Refactor / Self-Defending Layer Guard / Route-Inventory Guard / Security Hardening) / `v7.4` (Pipeline Health Endpoint / x402 Replay Protection / payment_bypassed Audit Log / CI Loop Trust Fix) / `v7.3` (Paid MCP Gateway / ACP Protocol / Zero-Trust Billing Audit Chain / Turso Distributed SQLite) / `v7.2` / v7.1 / v7.0 / v5.3 / v5.2 / v5.1.
...
- **GSAM package (GSAM-01…07, v7.7)**: `warden/gsam/` — Global Statistic Agentic Marketplace. ClickHouse holds the append-only observation stream ONLY; drift baselines / leases / quarantine journal / hourly rollups live in the SQLite `gsam` Turso DB; Redis holds quarantine flags + lease approval tokens (all with in-process fallbacks). **GDPR: observations are metadata-only** — `warden/gsam/ingest.py` is the sole producer→collector bridge and maps every payload through an explicit field ALLOWLIST; free-text fields (`detail`, `query_preview`) are never copied. Producers only do dict-build + `Queue.put_nowait` (µs, never blocks). Everything is **fail-open EXCEPT JIT leasing**, which is **fail-CLOSED**: with `gsam_lease_secret` unset every lease op raises `LeaseUnavailableError` → HTTP 503 (a credential path must never fail open); the signing secret never appears in any response body, and the approval token is delivered via Slack only. Drift uses EWMA over a weighted-cosine distance; crossing `gsam_drift_quarantine_threshold` (0.85) quarantines the agent, denied at both the marketplace (`dispatch_action`, after the Brand Agent gate) and Digital Staff (`staff_dispatch`, ADDED alongside — never replacing — the STAFF-01 boundary + STAFF-02 velocity checks). Read APIs (`/gsam/heatmap|agents/{id}/stats|compliance/score`) and the `gsam_agent_stats` semantic model read the SQLite rollup (never ClickHouse) so they work with ClickHouse disabled/down. Router `warden.gsam.api` is in `OPTIONAL_ROUTERS`; `gsam_enabled` feature key is Pro+. Tests run without Docker/ClickHouse/Redis (`GSAM_CLICKHOUSE_ENABLED=false`).
