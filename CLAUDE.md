...
- **Canary coverage is a gate, not a hope (OB-F12)**: every entry in `poison.CANARY_EXAMPLES` must score ≥ `CANARY_MIN_SCORE` (0.70) against `semantic._JAILBREAK_CORPUS`. `warden/tests/test_corpus_canary_coverage.py` (marked `slow`, runs in the CI coverage job) enforces both directions — canaries covered, benign controls held under 0.60. Editing either list without running it re-opens a permanently-firing `critical` alert. Canaries must be reached through genuine coverage, never by pasting a paraphrase of the canary into the corpus: that makes the check measure its own input.
- **Self-Healing snapshot bootstrap (OB-F12)**: a snapshot was only written on a HEALTHY cycle and only read on an UNHEALTHY one, so a corpus that was never healthy never got one and every rollback was a no-op. `CorpusHealthMonitor.run()` now seeds from the shipped corpus when no `.npz` exists. `alert_corpus_rollback` fires on the healthy→degraded transition only (`self._was_healthy`), not once per monitor interval.
