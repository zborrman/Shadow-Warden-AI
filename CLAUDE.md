...
- **Mutation testing**: NOT a CI gate — `mutmut run` is commented out in `ci.yml` ("too slow for CI minutes"). SR-7.3 (2026-08-03): the `[tool.mutmut]` config was stranded on 2.x syntax after `warden/constraints.txt` moved to `mutmut==3.6.0`, making local runs silently non-functional for an unknown period; migrated to the 3.x schema and extended from 2 to 5 modules (+ `auth_guard.py`, `causal_arbiter.py`, `marketplace/x402_gate.py`). Two more bugs found and fixed by actually executing it, not just reading the diff: (1) `also_copy` must list every module transitively imported by the mutated files or their tests — mutmut's `mutants/` sandbox is an isolated copy, not merged with the real source tree, so a bare `warden/tests/` entry left every module erroring on `ModuleNotFoundError` before a single mutant ran; (2) **never run mutmut against this repo's own built image** — `warden/Dockerfile` sets `ENV PYTHONPATH=/` + `WORKDIR /warden`, and that baked `/warden` (a regular package) wins Python's namespace-package resolution over the `mutants/` overlay outright, so every mutant silently tests unmutated code and reports as trivially "killed" (confirmed: baked `/warden/secret_redactor.py` was byte-identical to source, zero mutant markers, while the suite still "passed"). Run it in a plain interpreter instead. No survivor-count threshold is asserted anywhere — the "20 surviving mutants" figure previously here was never checked against a real number. First real 5-module sweep (2026-08-03): **1482 mutants, 696 killed / 786 survived**, concentrated almost entirely in the two newly-extended modules — `causal_arbiter.py` 405/652 (62%), `marketplace/x402_gate.py` 259/459 (56%) — while the pre-existing modules are healthy (`secret_redactor.py` 33/137 with registry-invariant tests already added in a prior slice, `semantic_guard.py` 0/7, `auth_guard.py` 89/227). Highest-value finding: in `causal_arbiter.calibrate_from_logs`, every argument of every `_drift_ok(...)` call in the final `and`-chained gate can be swapped for `None`/reordered/mislabeled without any test noticing — no test reaches the point where an earlier chain link passes and a later CPT parameter's specific drift value is what's actually verified, which is exactly the coordinated-poisoning scenario the CPT drift gate (#6 above) exists to stop. Remediation is untouched — this note records what mutation testing found, not a fix.
