...
**Version:** 7.9 · **License:** Proprietary · **Language:** Python 3.11+ · **Updated:** 2026-08-03
...
| `warden/Dockerfile` | **Multi-stage (P-3a)**: builder compiles liboqs + installs deps `-c constraints.txt`; runtime is `python:3.11-slim` with no compiler. CPU-only torch (build-asserted), non-root UID/GID 10001 |
...
| `warden/crypto/pqc.py` | Post-Quantum Cryptography — HybridSigner (Ed25519+ML-DSA-65), HybridKEM (X25519+ML-KEM-768), CryptoBackend, `pqc_selfcheck()`; liboqs fail-open in code, build-asserted in the image |
...
| `warden/billing/pricing.py` | **Canonical price list (FM-7)** — `TIER_PRICE_USD_MONTH`, `TIER_ALIASES`/`canonical_tier()`, `ANNUAL_DISCOUNT`, `TARGET_GROSS_MARGIN`, and **derived** `ANNUAL_PRICING`. Pure data, no warden imports (so `finops` can import it without a cycle) |
...
| `warden/billing/router.py` | Billing API — tier catalog (Pro $99.99/Enterprise $249), addon catalog+checkout+grant+revoke, `GET /billing/margin` (revenue vs. MTD LLM cost), `_calculate_overage()` |
| `warden/finops/llm_budget.py` | Per-tenant monthly LLM allowance + soft model gate (FM-7) — `tier_llm_budget_usd`, `mtd_spend_usd`, `budget_status`, `choose_model` (fail-open, never below `candidates[0]`), `choose_platform_model`/`record_platform_cost` (platform cost centre), `month_start_ts`, `cache_savings_mtd_usd`, `margin_report` |
| `warden/billing/overage_ledger.py` | Idempotent overage settlement (FM-7) — `billing_overage_charges` keyed on `(tenant_id, period)`; `settle()`, `get_charge()`, `list_period()`; `OVERAGE_CHARGE_ENFORCED` gates whether a charge is presented to a provider |
| `scripts/finops_report.py` | FinOps CLI — unit economics per tier, per-model turn cost, compose mem-limit audit + M/G/1 capacity ceiling (`--json` for machine use) |
...
| `warden/staff/economics.py` | Unit Economics Token Tracker (v7.2) — `TokenCostTracker` SQLite per-action LLM cost (Haiku/Sonnet/Opus pricing, `cached_tokens` at 10%); `get_report()`, `get_margin_alerts()`, `get_total_cost()`, `get_cost_since()`. Every LLM path in the product records here |
...
# Grafana is bound to 127.0.0.1:3001 (OB-7) — it holds every datasource
# credential and the alerting config, so it never goes on the public origin
# IP. Tunnel: ssh -L 3001:127.0.0.1:3001 root@<host>. The old value here
# named port 3000, which compose has never published.
NEXT_PUBLIC_GRAFANA_URL=http://127.0.0.1:3001
...
- **One chokepoint for money movement (FT-6, in progress)**: `warden/payments/authorize.py::authorize_payment()` mirrors `agentic_gate()`'s role but for payments — composes the autonomy check (`marketplace/autonomy.py::check_action()`) + Budget Guardian (`business_community/agentic_commerce/semantic_budget.py::check_budget()`) unconditionally, plus AP2 mandate verification when a `mandate_id` is given. Verdict precedence DENY > REQUIRE_APPROVAL > ALLOW; each check fails soft to REQUIRE_APPROVAL on its own error (never silent ALLOW, never hard DENY on an infra error). Opt-in via `AUTHORIZE_PAYMENT_ENFORCED` (default false). **Not yet a ratchet-enforced chokepoint** — `marketplace/listing.py::purchase_listing()` and `marketplace/clearing.py::clear()` both call it now; `credits.py::purchase_credits()` is deliberately excluded (it fulfills an already-settled external Lemon Squeezy payment, not an agent spend decision — see `warden/marketplace/CLAUDE.md`). Order-model consolidation (the last FT-6 sub-part, see `docs/order-model-consolidation-plan.md`) has shipped Phases A+B: `marketplace_purchases` has nine new nullable columns + `asset_id` relaxed to nullable (SQLite table rebuild, no `ALTER COLUMN`), and `m2m_store`/`agentic_commerce` now dual-write a mirrored fielded row via `warden/marketplace/listing.py::upsert_mirrored_order()` after their own blob-table write succeeds. `m2m_orders`/`commerce_orders`/`commerce_receipts` remain the source of truth — **every reader still queries them, not the mirror**. Do not read `marketplace_purchases` for m2m_store/agentic_commerce data yet; the mirror is fail-soft best-effort, not yet trusted. Phase C (cutover reads) hasn't started. Do not assume every money endpoint passes through `authorize_payment()` yet. **Enforcement prerequisite (shipped):** flipping `AUTHORIZE_PAYMENT_ENFORCED=true` used to block 100% of purchases and clearings, because `autonomy.check_action()` returns REQUIRE_APPROVAL for an agent with no policy and *nothing outside the admin endpoint ever created one*. `kya.py::screen_agent()` now grants a conservative L2 default via `autonomy.ensure_default_policy()` on VERIFIED (and `revoke_agent()` deletes it); `_DEFAULT_ALLOWED_ACTIONS` also gained `"purchase"`, the action string `listing.py` actually passes. The flip is now a real posture decision rather than a kill switch — see `warden/marketplace/CLAUDE.md` rule #26.
...
- **Base image — `python:3.11-slim`, browser sandbox deferred**: the gateway no longer runs on `mcr.microsoft.com/playwright/python:v1.49.0-noble` (~1.8 GB); it was dropped for disk headroom on the VPS and `BROWSER_ENABLED=false`. The old "do NOT switch to `python:3.x-slim`" rule applied only while Playwright was in-image and is superseded. **To re-enable the browser sandbox** (`warden/tools/browser.py`, `visual_assert_page`, `visual_diff`, `sova_visual_patrol`) you need the OS-level browser deps back: switch the *runtime* stage `FROM` to the MCR image, add `RUN playwright install chromium`, and set `BROWSER_ENABLED=true` — do not simply flip the env var, it will fail at launch. Non-root user uses GID/UID 10001 (1001 is taken by the noble base image, so keep 10001 if you switch back).
- **The site answers machines on the same URL (agent readiness, 2026-08-28)**: `Accept: text/markdown` is negotiated on the page URL itself, per [acceptmarkdown.com](https://acceptmarkdown.com). The decision lives in **one** module, `edge/negotiate.mjs` (`planRequest`/`planAfterProbe`/`chooseVariant`), which `middleware.ts` runs on Vercel (www) and `edge/preview-server.mjs` runs in CI; `docker/Caddyfile` implements the conservative subset for the apex. Rules: q-values decide, `*/*` means HTML (never markdown — that keeps curl and crawlers on today's behaviour), both variants carry `Vary: Accept`, and a client that accepts neither gets `406` with a markdown body. Markdown bodies live in `site/public/*.md`, one per route in `MARKDOWN_ROUTES`; adding a route means adding the file, the map entry, the page's `markdown=` prop, and the apex matcher — `warden/tests/test_site_agent_readiness.py` fails on any of the four being missed. `.vercelignore` excludes `*.md`; the negations for `site/public` are what keep the corpus deployable.
- **A missing path must say 404 on both surfaces**: the apex vhost has **no** `/index.html` `try_files` fallback (it made every unknown URL a 200 of the homepage, which told crawlers every invented path existed, and hid a dead `/early-access` CTA for months); `handle_errors` serves `404.html` with `status {err.status_code}`. Both 404 bodies — HTML and `/404.md` — must list the recovery routes (`llms.txt`, sitemap, agent instructions, `/doc`, `openapi.json`).
- **JSON-LD identity on every page**: both layouts (`BaseLayout.astro`, `Layout.astro`) emit an `@graph` with Organization + WebPage and a trailing-slash-normalised `rel="canonical"` (vercel.json redirects `/x/` → `/x`, so an un-normalised canonical names a 308). The homepage adds `SoftwareApplication` with an `AggregateOffer`; its `lowPrice`/`highPrice` are pinned to `warden.billing.pricing` by `test_pricing_coherence.py` like every other site price.
- **`warden` CLI**: console script of the published `shadow-warden-sdk` (`sdk/python`, `shadow_warden.cli:main`). Exit codes are the contract — 0 allowed, 1 blocked, 2 usage, 3 gateway unreachable — so a 500 can never read as "blocked". CI installs the package and runs `warden version`, because a console script that is only declared is not a CLI. Publishing a release is a credentialled step and is not automated.
...
- **No root npm workspaces**: the repo root *does* have a `package.json`, but it exists only to supply `@vercel/edge` to `middleware.ts` (the Vercel Edge Middleware that re-asserts the real client IP across the www→Vercel→api rewrite) — do not delete it. It deliberately has **no `workspaces` key**, and must not gain one: that breaks `npm ci` in subdirectories on Linux npm v10. `portal/`, `dashboard/` and `site/` are standalone npm projects with their own lock files; `packages/ui/` has its own `package.json` but is not a workspace member.
- **`landing/` is generated, and two surfaces serve it differently**: Vercel builds `site/` itself (`vercel.json` → `buildCommand`/`outputDirectory: site/dist`), while the VPS mounts the committed `landing/` at `/srv/landing` for Caddy (`docker-compose.yml`). So `landing/` must stay committed *and* must never drift from `site/`. CI enforces it (`Check landing/ matches a fresh build`) — a PR that edits `site/` without rebuilding fails. Rebuild with `cd site && npm ci && npm run build && cd .. && rm -rf landing && cp -r site/dist landing`. Do **not** reintroduce a bot that publishes it after merge: `main` is protected, so a bot push is rejected, and a bot branch pushed with `GITHUB_TOKEN` never triggers the required checks, leaving its PR blocked forever.
...
- **Canaries measure drift, not an absolute score (OB-F12)**: the canary check compares each `poison.CANARY_EXAMPLES` entry against the score it had at process start (`_canary_baseline`, recorded once in `_initialise_sync`, never re-recorded — re-recording is a slow-boil poisoning path). Degraded when a canary falls `CANARY_DRIFT_TOLERANCE` (0.10) below its baseline, or under `CANARY_ABSOLUTE_FLOOR` (0.30) for a wiped corpus. The old absolute 0.70 floor was unsatisfiable: 7/10 failed from day one, and above ~0.70 MiniLM cosine is a *paraphrase* relation — sixteen genuinely distinct attacks in the same families moved the canaries by nothing (closest neighbour 0.6559), so only restatements of the canaries would lift them, making the check measure its own input. **Never add a canary paraphrase to `_JAILBREAK_CORPUS` and never raise the floor to "fix" a failing canary.** `warden/tests/test_corpus_canary_coverage.py` (`slow`, runs in the CI coverage job) gates it, including that the check can still fail on a poisoned corpus.
- **Self-Healing snapshot bootstrap (OB-F12)**: a snapshot was only written on a HEALTHY cycle and only read on an UNHEALTHY one, so a corpus that was never healthy never got one and every rollback was a no-op. `CorpusHealthMonitor.run()` seeds from the shipped corpus when no `.npz` exists, or when the snapshot's `corpus_digest` does not match `semantic.SHIPPED_CORPUS_DIGEST` (a snapshot from an earlier build would roll coverage backwards silently). `alert_corpus_rollback` fires on the transition into canary failure only (`self._canaries_were_failing` — keyed to the canary condition, not `report.healthy`, so a drift cycle cannot mask the first canary failure after it), not once per monitor interval.
- **A metric queried by an alert must have a writer (OB-F12)**: prometheus exports a registered Gauge at 0 whether or not anything ever calls `.set()`, so a declared-but-unwritten gauge reads as a measurement, not as missing data. `warden/tests/test_alert_metrics_have_writers.py` hard-fails any `warden_*` metric an alert rule queries that no non-test module references. `warden_community_peering_connections` was that defect (`refresh_peering_gauge()` in `communities/peering.py` now counts it from the table, at boot and on accept/revoke — not `inc()/dec()`, which a restart zeroes).
...
- **RateLimit headers on every response**: `warden/middleware/rate_limit_headers.py` publishes `RateLimit-Policy` (always) and `RateLimit` (only when the route consumed the window, i.e. slowapi recorded `request.state.view_rate_limit`), plus the draft-06 spelling `RateLimit-Limit`/`-Remaining`/`-Reset` and the legacy `X-RateLimit-*`. `Retry-After` on 429 only. Pure ASGI, registered **inside** `APIVersionMiddleware` (which must stay `stack[0]` — `test_api_versioning.py` asserts it) and **outside** `QuotaMiddleware`, which stashes the monthly reading on `scope["state"]["quota_policy"]` so the header layer does not repeat the SQLite + Redis work. The one bucket read runs through `asyncio.to_thread` — `limits` ships a *synchronous* Redis client and this middleware sits on the response path. Never publish a "remaining" for a route that consumed nothing: there is no bucket behind it. Kill switch `RATELIMIT_HEADERS_ENABLED=false`.
- **MCP protocol versions are asserted, not claimed**: `warden/mcp/gateway.py::SUPPORTED_PROTOCOL_VERSIONS = ("2025-06-18", "2025-03-26", "2024-11-05")`. `2026-07-28` is **deliberately absent** — it replaced the `initialize` handshake with per-request `_meta` negotiation and a mandatory `server/discover`, neither of which is implemented. `warden/tests/test_mcp_manifest.py` fails on any version string outside the set of revisions that actually exist (the descriptor once advertised `2025-11-05`, which never has). `/.well-known/mcp.json` is generated from the live tool tables by `warden/discovery/well_known.py` and proxied to www (`vercel.json`) and the apex (`docker/Caddyfile`) — three surfaces must agree.
- **MCP free product tools**: `warden/mcp/product_tools.py` exposes `filter_text`, `gateway_health`, `list_pricing` ahead of the twelve paid staff tools, bypassing the x402 gate *and* the DPI scan. `filter_text` self-calls `POST /filter` over loopback with the **caller's own** `X-API-Key` (never the internal key) so auth, quota, rate limit, ERS and audit all apply exactly once; charging an x402 fee on top would bill twice for one request. DPI is skipped because `filter_text` *is* the inspector — scanning its argument would refuse the payload it was called to judge.
- **`site/public/openapi.json` is generated, not hand-synced**: run `python scripts/export_openapi.py` after any route or description change, then rebuild `landing/`. It drifted to v5.6.0/562 paths against a v7.9.0/658-path gateway, omitting `/mcp/` — which the site's own developer portal links. `warden/tests/test_site_agent_readiness.py` fails when the published `info.version` falls behind `warden.__version__`. The live app now declares `servers` (it did not), so a client reading the spec from the marketing host still learns the API base URL.
- **Organization JSON-LD asserts only the country**: both layouts publish `address: PostalAddress{addressCountry: 'IL'}` and `contactPoint` entries carrying an email. Street, locality and postal code are not published anywhere this repo can verify, and an invented address in machine-readable identity data is what a buyer's compliance review checks. `test_no_contact_address_is_invented` restricts the emails to mailboxes the human pages already publish.
...
- **One price list (FM-7)**: `warden/billing/pricing.py` owns `TIER_PRICE_USD_MONTH`, the tier-alias table, and the **derived** annual prices (`monthly × 12 × (1 − ANNUAL_DISCOUNT)`). `feature_gate.ANNUAL_PRICING`, `billing/router.py`'s catalog and `finops/margin._TIER_PRICE_USD_MONTH` all re-export from it — never re-type a price, and never hand-write an annual figure. Three independent copies had drifted a full tier apart, so a year of Pro sold for $703 against a $1 199.88 list (a 41% discount on a plan advertised at 15%) while the margin engine rated revenue off a third number. Guarded by `warden/tests/test_pricing_coherence.py`.
- **LLM COGS is measured, then gated (FM-7)**: every LLM call must reach `warden/staff/economics.py::TokenCostTracker.record()` with its `cached_tokens` — MasterAgent and the Evolution Engine recorded *nothing* until FM-7, and they are the two most expensive paths in the product. `warden/finops/rating.py::price_for()` resolves exact id → **model-family prefix** → default, because `claude-opus-4-6` was absent from `PRICE_BOOK` and rated as Sonnet (5× too cheap); never let a new dated snapshot fall through to the default. `warden/finops/llm_budget.py` derives a per-tier monthly inference allowance from the list price (`price × (1 − TARGET_GROSS_MARGIN)`) and picks a model within it: **soft, additive, fail-open** — never blocks, never routes below `candidates[0]`, never returns a model the caller did not offer, and resolves to the *most capable* model on any fault (a FinOps outage must not degrade a paying tenant). Enterprise + unresolvable plans are uncapped. `EVOLUTION_DAILY_BUDGET_USD` (default $5) bounds evolution spend — its rate gate caps calls per window, which sustained is ~$750/day. `GET /billing/margin` reports revenue vs. MTD cost; `scripts/finops_report.py` prints unit economics + node capacity. Plan: `docs/monetization-plan.md`.
- **Metered units have one price each (FM-7 slice 2)**: a marketplace search costs `pricing.MARKETPLACE_SEARCH_FEE_USD` on **both** rails — `marketplace/x402_gate.py` and `marketplace/credits.py` derive from it, after x402 spent months charging $0.000001 against a credit's $0.001 for identical work. An agent turn beyond a plan's `AGENT_TURN_ALLOWANCE` costs `AGENT_TURN_OVERAGE_USD`; the allowance is published in the tier catalog and metered by `_calculate_overage()`, and `test_pricing_coherence.py` asserts it both fits inside the tier's own LLM budget and is only non-zero where `master_agent_enabled` is true. Never publish an allowance a plan cannot fund.
- **Overage is settled through a ledger, never straight to a provider (FM-7 slice 2)**: `warden/billing/overage_ledger.py::settle()` is idempotent per `(tenant_id, period)` — the primary key — so a re-run or retry of `sova_overage_billing` cannot double-charge. Presenting a charge is gated behind `OVERAGE_CHARGE_ENFORCED` (default **false**, a deliberate posture flip like `AUTHORIZE_PAYMENT_ENFORCED`); off, charges store as `computed` with the audit trail and no money moves. A charge that cannot be presented stores as `failed` — never silently as `charged`. Before FM-7 the cron computed a number, Slacked it, and collection was delegated to an admin endpoint nothing called.
- **Platform LLM work has its own cost centre (FM-7 slice 2)**: work that serves the platform rather than one customer — `rag_evolver`, `integrations/nemo_bridge`, `agent/red_team` — calls `finops/llm_budget.py::choose_platform_model()` and `record_platform_cost()`, budgeted by `PLATFORM_LLM_MONTHLY_BUDGET_USD` (default $50). They have no plan to derive an allowance from, so without this they are uncapped. The Evolution Engine keeps its own `EVOLUTION_DAILY_BUDGET_USD` plus a per-tenant fairness share (`EVOLUTION_TENANT_SHARE`, default 0.5 of a rate window) so one free-tier attacker cannot starve paying tenants of new rules.
- **Tier prices**: Starter $0 (1k req), Individual $5/mo (5k req), Community Business $39.99/mo (15k req, File Scanner + Shadow AI Monitor + 3 communities×10 members + 180-day retention), Pro $99.99/mo (50k req, MasterAgent included), Enterprise $249/mo (unlimited, PQC + Sovereign). Add-ons: `shadow_ai_discovery` +$15/mo (Pro+), `xai_audit` +$9/mo (Individual+). MasterAgent is included in Pro — **not sold as an add-on**.
...
- **PQC optional in code, GUARANTEED in the image (P-3a)**: `warden/crypto/pqc.py` wraps liboqs with `_OQS_AVAILABLE`; all PQC paths raise `PQCUnavailableError(RuntimeError)` when it is missing, and classical Ed25519/X25519 still work — that fail-open is deliberate, for air-gapped and sub-Enterprise deployments. **But the shipped image must never be one of them.** `pip install liboqs-python` installs the Python *bindings only*; the native library is a separate CMake build. For every release from v4.7 (`967303cd`) to v7.7 that build was never run, the Dockerfile step ended in `|| echo`, and `pqc.py` reported the resulting `RuntimeError` as *"liboqs-python not installed"* — so PQC, a **sold Enterprise feature**, silently degraded to classical Ed25519 in production for months with no reader. `warden/Dockerfile` now compiles liboqs in the builder stage and **fails the build** unless `ML-DSA-65` + `ML-KEM-768` load. `LIBOQS_VERSION` **must** match the pinned `liboqs-python` — `oqs.py` derives `OQS_VERSION` from its own package version and will try to download and build that exact tag *at import time* otherwise. Never restore a `|| echo` / `|| true` on a crypto build step. `pqc_selfcheck()` checks the mechanisms are *enabled*, not merely importable, and runs in the lifespan + `GET /health/pipeline` (`pqc` key — deliberately **not** in `degraded_stages`).
...
- **Hybrid signature layout**: 3373 bytes = Ed25519 sig (64 B) + ML-DSA-65 sig (3309 B). `HybridSignature.pack()` / `unpack()` handle serialization. `hybrid_verify()` falls back to Ed25519-only if liboqs unavailable — **this is the fail-open that hid the broken build**; the image-level build assertion, not this function, is what guarantees a real hybrid signature. Verified in production 2026-07-27: pubkey 1952 B, packed sig 3373 B, tampered payload rejected.
...
- **Mutation testing**: NOT a CI gate — `mutmut run` is commented out in `ci.yml` ("too slow for CI minutes"). SR-7.3 (#277, 2026-08-03): `[tool.mutmut]` had been silently broken since `warden/constraints.txt` moved to `mutmut==3.6.0` (the config was still 2.x schema); fixed and extended from the original 2 modules to 5 (+ `auth_guard.py`, `causal_arbiter.py`, `marketplace/x402_gate.py`). First real sweep: **1482 mutants, 786 survived**. Not evenly spread — `secret_redactor.py` was brought to 19/module (under the documented threshold) with direct unit tests for previously-only-indirectly-exercised helpers; `semantic_guard.py` and `auth_guard.py` are comparatively healthy; `causal_arbiter.py` (405 survivors) and `marketplace/x402_gate.py` (259 survivors) — the two modules newly added by this extension — carry nearly all the remaining volume and are open follow-on work. One finding from that sweep is concrete enough to act on directly: in `causal_arbiter.calibrate_from_logs`, every argument of every `_drift_ok(...)` call in the final `and`-chained gate (covering `obfusc_pos`/`obfusc_neg`/`ers_center`/`entropy_center`) can be replaced with `None`, have its label dropped, or be reordered without any test noticing — meaning no test currently reaches the point where an earlier link in the chain passes and a *later* CPT parameter's specific drift value is what's actually being verified. That is exactly the "slow-burn coordinated poisoning" scenario the CPT drift gate (#6 above) documents itself as existing to stop. **Remediated 2026-08-11** by `warden/tests/test_cpt_drift_gate_links.py`: one corpus, varied priors, each reachable link rejecting in isolation with the later links satisfied, plus bracketed-label assertions (`CPT[ers_center]` — a bare substring check passes against a mutated `XXers_centerXX`) and below-prior drift values so argument order is observable. Verified by hand-applying five representative mutants; all five now fail. **The sweep also found that the gate's fourth link is dead:** `entropy_center` is clamped to [3.5, 5.5] and its shift is bounded to ±0.3, so its maximum achievable drift is **13.76%** against a 25% threshold — `_drift_ok` can never reject it. That parameter is bounded by the clamp, not by the gate, and can be walked to a clamp edge across runs unopposed (at `entropy_center=5.5` a high-entropy payload stops looking unusual). Per-run drift caps do not stop a walk; that is a detection-math decision for Track B, recorded rather than decided. The ~786 total survivors remain a separate effort.
